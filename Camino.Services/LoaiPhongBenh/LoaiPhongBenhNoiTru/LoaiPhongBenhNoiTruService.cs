...
{
    [ScopedDependency(ServiceType = typeof(ILoaiPhongBenhNoiTruService))]
    public class LoaiPhongBenhNoiTruService
        : MasterFileService<Core.Domain.Entities.LoaiPhongBenh.LoaiPhongBenhNoiTrus.LoaiPhongBenhNoiTru>,
        ILoaiPhongBenhNoiTruService
    {
        public LoaiPhongBenhNoiTruService
            (IRepository<Core.Domain.Entities.LoaiPhongBenh.LoaiPhongBenhNoiTrus
                .LoaiPhongBenhNoiTru> repository)
            : base(repository)
        { }
        ...
        public async Task<GridDataSource> GetDataForGridAsync(QueryInfo queryInfo, bool forExportExcel)
        {
            ...
            var query = BaseRepository.TableNoTracking
                .Select(s => new LoaiPhongBenhNoiTruGridVo
                {
                    Id = s.Id,
                    Ten = s.Ten,
                    MoTa = s.MoTa
        ...
        {
            var result = BaseRepository.TableNoTracking.Select(s => new LoaiPhongBenhNoiTruGridVo
            {
                Id = s.Id,
                Ten = s.Ten,
                MoTa = s.MoTa,
