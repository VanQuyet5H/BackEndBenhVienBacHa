...
namespace Camino.Services.BaoCao
{
    public partial class BaoCaoService
    {
        ...
        public async Task<List<LookupItemVo>> GetKhoDuocPhamLookupAsync(LookupQueryInfo queryInfo)
        {
            var lookup = await _khoRepository.TableNoTracking.Where(s => s.LoaiDuocPham == true && (s.LoaiKho == EnumLoaiKhoDuocPham.NhaThuoc
                                                                                              || s.LoaiKho == EnumLoaiKhoDuocPham.KhoTongDuocPhamCap1
                                                                                              || s.LoaiKho == EnumLoaiKhoDuocPham.KhoTongDuocPhamCap2
        ...
        public async Task<GridDataSource> GetDataBaoCaoTinhHinhTraNCCForGridAsync(BaoCaoTinhHinhTraNCCQueryInfo queryInfo)
        {
            var dataXuatKhos = _xuatKhoDuocPhamRepository.TableNoTracking
                ...
                { 
                    ...
                    o.SoPhieu, 
                    o.NhaThauId,
                    XuatKhoDuocPhamChiTiets = o.XuatKhoDuocPhamChiTiets
                        .Select(ct => new 
                        { 
                            ct.DuocPhamBenhVienId,
                            XuatKhoDuocPhamChiTietViTris = ct.XuatKhoDuocPhamChiTietViTris.Select(vt => new
                            {
                                vt.Id,
                                vt.SoLuongXuat,
                                vt.NhapKhoDuocPhamChiTiet.Solo,
                                vt.NhapKhoDuocPhamChiTiet.DonGiaNhap,
                            }).ToList()
                        }).ToList() 
                }).ToList();
            ...
            var duocPhamBenhVienIds = dataXuatKhos.SelectMany(o => o.XuatKhoDuocPhamChiTiets).Select(o=>o.DuocPhamBenhVienId).ToList();

            var thongTinNhaThau = _nhaThauRepository.TableNoTracking.Where(o => nhaThauIds.Contains(o.Id)).Select(o => new { o.Id, o.Ten }).ToList();

            var thongTinHoaDon = _yeuCauNhapKhoDuocPhamRepository.TableNoTracking
                .Where(o => o.DuocKeToanDuyet == true && soChungTus.Contains(o.SoChungTu) && o.YeuCauNhapKhoDuocPhamChiTiets.Any(ct => nhaThauIds.Contains(ct.HopDongThauDuocPham.NhaThauId)))
                .Select(o => new 
                { 
                    o.SoChungTu, 
                    o.NgayHoaDon, 
                    NhaThauIds = o.YeuCauNhapKhoDuocPhamChiTiets.Select(ct => ct.HopDongThauDuocPham.NhaThauId).ToList()
                });

            var thongTinDuocPham = _duocPhamBenhVienRepository.TableNoTracking
            ...
            var allData = new List<BaoCaoTinhHinhTraNCCGridVo>();
            foreach (var dataXuatKho in dataXuatKhos)
            {
                foreach(var xuatKhoDuocPhamChiTiet in dataXuatKho.XuatKhoDuocPhamChiTiets)
                {
                    foreach (var xuatKhoDuocPhamChiTietViTri in xuatKhoDuocPhamChiTiet.XuatKhoDuocPhamChiTietViTris)
                    {
                        var duocPham = thongTinDuocPham.First(o => o.Id == xuatKhoDuocPhamChiTiet.DuocPhamBenhVienId);

                        var baoCaoTinhHinhTraNCCGridVo = new BaoCaoTinhHinhTraNCCGridVo
                        {
                            Id = xuatKhoDuocPhamChiTietViTri.Id,
                            ...
                            MaThuoc = duocPham.Ma,
                            TenThuoc = duocPham.Ten,
                            DVT = duocPham.DonViTinh,
                            SoLo = xuatKhoDuocPhamChiTietViTri.Solo,
                            SoLuongTra = xuatKhoDuocPhamChiTietViTri.SoLuongXuat,
                            DonGiaNhap = xuatKhoDuocPhamChiTietViTri.DonGiaNhap,
                            Nhom = duocPham.DuocPhamBenhVienPhanNhomId != null ? duocPhamBenhVienPhanNhoms.FirstOrDefault(o=>o.Id == duocPham.DuocPhamBenhVienPhanNhomId)?.Ten : "CHƯA PHÂN NHÓM"
                        };
            ...
            var returnData = allData
                .GroupBy(o => new { o.SoHoaDon, o.NgayHoaDon, o.SoPhieuTra, o.NgayTra, o.MaThuoc, o.SoLo, o.DonGiaNhap }, o => o,
                    (k, v) => new BaoCaoTinhHinhTraNCCGridVo
                    {
                        Id = v.First().Id,
        ...
        public virtual byte[] ExportBaoCaoTinhHinhTraNCC(GridDataSource gridDataSource, BaoCaoTinhHinhTraNCCQueryInfo query)
        {
            var datas = (ICollection<BaoCaoTinhHinhTraNCCGridVo>)gridDataSource.Data;
            ...
            using (var stream = new MemoryStream())
            {
                ...
                {
                    var worksheet = xlPackage.Workbook.Worksheets.Add("BẢNG KÊ TÌNH HÌNH TRẢ NHÀ CUNG CẤP");
                    //set row
                    ...
                    using (var range = worksheet.Cells["A8:N8"])
                    {
                        ...
                        range.Worksheet.Cells["G8"].Value = "Mã thuốc";

                        range.Worksheet.Cells["H8"].Style.Border.BorderAround(ExcelBorderStyle.Thin, Color.Black);
                        range.Worksheet.Cells["H8"].Value = "Tên thuốc, hàm lượng";
                    ...
                    int index = 9; // bắt đầu đổ data từ dòng 9
                        ...
                        foreach (var nhom in listNhom)
                        {
                            ...
                            if (listTheoNhom.Any())
                            {
                                using (var range = worksheet.Cells["A" + index + ":N" + index])
                                {
                                    ...
                                    //range.Worksheet.Cells["M" + index].Value = listTheoNhom.Sum(s => s.ThanhTien);
                                ...
                                foreach (var item in listTheoNhom)
                                {
                                    using (var range = worksheet.Cells["A" + index + ":N" + index])
                                    {
                                        ...
                                        range.Worksheet.Cells["G" + index].Value = item.MaThuoc;

                                        range.Worksheet.Cells["H" + index].Style.Border.BorderAround(ExcelBorderStyle.Thin, Color.Black);
                                        range.Worksheet.Cells["H" + index].Style.HorizontalAlignment = ExcelHorizontalAlignment.Left;
                                        range.Worksheet.Cells["H" + index].Value = item.TenThuoc;
