...
namespace Camino.Services.BaoCao
{
    ...
    {
        public async Task<List<LookupItemVo>> GetKhoVTYTBaoCaoChiTietXuatNoiTruLookupAsync(LookupQueryInfo queryInfo)
        {
        ...
        public async Task<GridDataSource> GetDataBaoCaoChiTietXuatNoiBoForGridAsync(BaoCaoChiTietXuatNoiBoQueryInfo queryInfo)
        {
            var xuatKhoVatTuQuery = _xuatKhoVatTuRepository.TableNoTracking
                .Where(o => o.KhoXuatId == queryInfo.KhoId && o.KhoNhapId != null && o.KhoNhapId != queryInfo.KhoId && o.NgayXuat >= queryInfo.FromDate && o.NgayXuat < queryInfo.ToDate);

            var xuatKhoVatTuData = xuatKhoVatTuQuery.Select(o => new
            {
                KhoNhap = o.KhoVatTuNhap.Ten,
                KhoNhapId = o.KhoNhapId,
                ChiTietVatTus = o.XuatKhoVatTuChiTiets
                .SelectMany(x => x.XuatKhoVatTuChiTietViTris)
                    .Select(y => new BaoCaoChiTietXuatNoiBoGridVo
                    {
                        Id = y.Id,
                        ...
                        DonGia = y.NhapKhoVatTuChiTiet.DonGiaBan
                    }).ToList()
            }).ToList();


            var baoCaoChiTietXuatNoiBoGridVos = new List<BaoCaoChiTietXuatNoiBoGridVo>();
            foreach (var xuatKhoVatTu in xuatKhoVatTuData)
            {
                var chiTiets = xuatKhoVatTu.ChiTietVatTus.ToList();
                for (int i = 0; i < chiTiets.Count; i++)
                {
                    chiTiets[i].Kho = xuatKhoVatTu.KhoNhap;
                }
                baoCaoChiTietXuatNoiBoGridVos.AddRange(chiTiets);
            }

            var dataReturn = baoCaoChiTietXuatNoiBoGridVos
                .GroupBy(o => new { o.Kho, o.VatTuBenhVienId, o.DonGia }, o => o,
                (k, v) => new BaoCaoChiTietXuatNoiBoGridVo
                {
                    Kho = k.Kho,
            ...
            //return new GridDataSource { Data = BaoCaoChiTietXuatNoiBoGridVos().ToArray(), TotalRowCount = BaoCaoChiTietXuatNoiBoGridVos().Count() };
        }

        public virtual byte[] ExportBaoCaoChiTietXuatNoiBo(GridDataSource gridDataSource, BaoCaoChiTietXuatNoiBoQueryInfo query)
        {
            var datas = (ICollection<BaoCaoChiTietXuatNoiBoGridVo>)gridDataSource.Data;
            ...
            using (var stream = new MemoryStream())
            {
                using (var xlPackage = new ExcelPackage(stream))
                {
                    ...
                    string[] SetColumnItems = XuLyDanhSachKyTuXuatNoiBo(countKho, 26).ToArray();
                    ...
                    using (var range = worksheet.Cells["A8:C8"])
                    {
                        ...
                        range.Worksheet.Cells["B8"].Value = "Tên VTYT (Hãng SX, Nước SX)";
                    ...
                    if (listThuoc.Any())
                    {
                        foreach (var item in listThuoc)
                        {
                            ...
                            for (int n = 0; n < countKho; n++)
                            {
                                ...
                                worksheet.Cells[index + 1, column].Style.Font.Bold = true;
        ...
        private List<string> XuLyDanhSachKyTuXuatNoiBo(int soKhoHoanTra, int tongSoKyTu)
        {
            var danhSachKyTuSauKhiXuKy = new List<string>();

            var danhSachKyTuPre = KyTuBCXuatNoiBos();
            var danhSachKyTuResult = KyTuBCXuatNoiBos();
            var khoBatDauTuKyTus = KhoNoiBoBatDauTuKyTus();
        ...
        private List<string> KyTuBCXuatNoiBos()
        {
        ...
        private List<string> KhoNoiBoBatDauTuKyTus()
        {
            var kyTus = new List<string>()
