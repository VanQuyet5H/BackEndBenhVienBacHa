...
namespace Camino.Services.BaoCao
{
    ...
    {
        private async Task<List<BaoCaoDoanhThuKhamDoanTheoKhoaPhongGridVo>> GetDataDoanhThuKhamDoanAsync(BaoCaoDoanhThuKhamDoanTheoKhoaPhongQueryInfo queryInfo, bool isTotalPage = false)
        {
            ...
            var results = new List<BaoCaoDoanhThuKhamDoanTheoKhoaPhongGridVo>();
            if (tuNgay != null && denNgay != null)
            {
                var thongTinKhams = _yeuCauTiepNhanRepository.TableNoTracking
                    ...
                    .Select(item => new BaoCaoDoanhThuKhamDoanTheoKhoaPhongGridVo()
                    {
                        Id = item.Id,
                    ...
                    if (results.Any())
                    {
                        ...
                        var lstDoanhThuDichVuKyThuat = _yeuCauDichVuKyThuatRepository.TableNoTracking
                            ...
                            .Select(item => new DoanhThuTheoDichVuVo()
                            {
                                ...
                                DoanhThu = item.DonGiaUuDai * item.SoLan,
        ...
        public async Task<GridDataSource> GetDataBaoCaoDoanhThuKhamDoanTheoKhoaPhongForGridAsync(QueryInfo queryInfo)
        {
        ...
        public async Task<GridDataSource> GetDataTotalPageBaoCaoDoanhThuKhamDoanTheoKhoaPhongForGridAsync(QueryInfo queryInfo)
        {
            var timKiemNangCaoObj = new BaoCaoDoanhThuKhamDoanTheoKhoaPhongQueryInfo();
            if (!string.IsNullOrEmpty(queryInfo.AdditionalSearchString) && queryInfo.AdditionalSearchString.Contains("{"))
            {
        ...
        public virtual byte[] ExportBaoCaoDoanhThuKhamDoanTheoKhoaPhongGridVo(GridDataSource gridDataSource, QueryInfo query)
        {
            var timKiemNangCaoObj = new BaoCaoDoanhThuKhamDoanTheoKhoaPhongQueryInfo();
            ...
            var lstKhoa = datas
                ...
                .Select(item => new DoanhThuTheoKhoaVo
                {
                    KhoaId = item.KhoaId,
            ...
            lstKhoa.Add(new DoanhThuTheoKhoaVo
            {
                KhoaId = -1,
                TenKhoa = "Tổng Cộng"
            });
            ...
            using (var stream = new MemoryStream())
            {
                ...
                {
                    var worksheet = xlPackage.Workbook.Worksheets.Add("BÁO CÁO TỔNG HỢP DOANH THU KHÁM ĐOÀN THEO KHOA/ PHÒNG");
