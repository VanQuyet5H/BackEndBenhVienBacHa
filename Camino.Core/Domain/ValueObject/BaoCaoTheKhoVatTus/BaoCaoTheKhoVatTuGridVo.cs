...
namespace Camino.Core.Domain.ValueObject.BaoCaoTheKhoVatTus
{
    public class BaoCaoTheKhoVatTuGridVo : GridItem
    {
        public BaoCaoTheKhoVatTuGridVo()
        {
            BaoCaoTheKhoVatTuChiTiets = new List<BaoCaoTheKhoVatTuChiTietGridVo>();
        }
        ...
        public List<BaoCaoTheKhoVatTuChiTietGridVo> BaoCaoTheKhoVatTuChiTiets { get; set; }
    }

    public class BaoCaoTheKhoVatTuThongTinXuatKhoDeNhapVe
    {
    ...
    public class BaoCaoTheKhoVatTuChiTietGridVo : GridItem
    {
        ...
        public string DienGiai => BenhNhanTraLai == true
            ? $"{TenKho} nhận hoàn trả từ bệnh nhân"
            : SLXuat.GetValueOrDefault() > 0
                ? ((LoaiXuatKho == Enums.XuatKhoDuocPham.XuatChoBenhNhan || LoaiXuatKhoVatTu == Enums.EnumLoaiXuatKho.XuatChoBenhNhan) ? (!string.IsNullOrEmpty(HoTenBenhNhan) ? $"Xuất cho bệnh nhân {HoTenBenhNhan} - mã TN: {MaTiepNhan}" : $"{TenKho} xuất cho bệnh nhân") : (!string.IsNullOrEmpty(XuatQuaKho) ? $"{TenKho} xuất nội bộ {XuatQuaKho}" : $"{TenKho} xuất khác"))
